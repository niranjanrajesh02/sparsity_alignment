...
from my_utils.imagenet import get_imagenet_dataloader, extract_labels
from my_utils.model_helpers import set_seed, init_model, get_layer_names, get_nice_layer_names, get_layer_activations
from my_utils.representations import compute_knn_acc
...
def expt_main(prune_method="random"):
    ...
    if prune_all:
        knn_acc_results_df = pandas.DataFrame(columns=[f'sparsity_{k}' for k in sparsity_levels]) # only need one row for full model results since we are not doing layerwise pruning
    else:
        knn_acc_results_df = pandas.DataFrame(columns=['layer_name'] + [f'sparsity_{k}' for k in sparsity_levels])
    ...
    for ki, k in enumerate(sparsity_levels):
        ...
        for li, ln in tqdm(enumerate(layer_names), total=len(layer_names), desc="Layerwise Evaluation"):
            ...
            knn_acc = compute_knn_acc(acts, labels)

            knn_acc_results_df.loc[li, 'layer_name'] = nice_ln
            knn_acc_results_df.loc[li, f'sparsity_{k}'] = knn_acc

        #save intermediate results  
        knn_acc_results_df.to_csv(os.path.join(results_path, f'vgg_knn_acc_{prune_method}_{target_suffix}.csv'), index=False)
