...
def expt_main(prune_method="random"):
    ...
    if prune_all:
        val_acc_results_df = pandas.DataFrame(columns=[f'sparsity_{k}' for k in sparsity_levels]) # only need one row for full model results since we are not doing layerwise pruning
    else:
    ...
    for ki, k in enumerate(sparsity_levels):
        ...
        for li, ln in tqdm(enumerate(layer_names), total=len(layer_names), desc="Layerwise Evaluation"):
            ...
            if not prune_all: # only compute & save layerwise eval accuracies if we are pruning layerwise
        ...
        if prune_all: # compute & save layerwise eval accuracies only once if we are pruning full model
            val_acc = eval_model_val(model_sparse, data_loader, device_id=config.device_id)
